...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
            ...
            this.yandexMapParams(parameters);
            ...
            if (this.result.DELIVERY_RADIUS) {
                this.deliveryRadius = this.result.DELIVERY_RADIUS * 1 * 1000 * 1;
            }
            if (this.result.COORDINATS) {
                this.coordinatsShopGps = this.result.COORDINATS;
                this.TITLE_MAP = this.result.TITLE_MAP;
            ...
            this.sendRequest('refreshOrderAjax',null,true);
        },

        /**
         * Send ajax request with order data and executes callback by action
         */
        sendRequest: function (action, actionData, isInit = false) {
            var form;
            ...
            if (action === 'saveOrderAjax') {
                ...
                BX.ajax({
                    ...
                    dataType: 'json',
                    url: this.ajaxUrl,
                    data: this.getData(action, actionData, isInit),
                    onsuccess: BX.delegate(function (result) {
                        ...
                        if (this.params.PICKUP_MAP_TYPE == 'google') {
                            this.radiusDostavkiMap();
                        } else {
                            this.yandexMapShow();
                        }
                    }, this),
        ...
        getData: function (action, actionData, isInit = false) {
            var data = {
            ...
            if (isInit) {
                var initDate = this.getDefaultDate();
                var initName = 'ORDER_PROP_' + initDate.id;
                data.order[initName] = initDate.value;
            }
            ...
            if (action === 'delete' || action === 'recalculate')
                data.id = actionData;
        ...
        initDefaultDate: function () {
            var curDate, curProp, i;

            for (i = 0; i < this.result.ORDER_PROP.properties.length; i++) {
                curProp = this.result.ORDER_PROP.properties[i];

                if (curProp.CODE == 'DATE') {
                    curDate = new Date();
                    curDate.setHours(curDate.getHours() + 1);
                    curProp.DEFAULT_VALUE = curDate.toLocaleString("ru-RU");
                    curProp.VALUE[0] = curProp.DEFAULT_VALUE;
                }
            }
        },
        ...
        getDefaultDate: function () {
            ...
            for (i = 0; i < this.result.ORDER_PROP.properties.length; i++) {
                ...
                if (curProp.CODE == 'DATE') {
                    res.id = curProp.ID;
                    res.value = curProp.DEFAULT_VALUE;
                    return res;
                }
            }
        },
        ...
        editActiveBasketBlock: function (activeNodeMode) {
            ...
            if (this.initialized.basket) {
                ...
                if (this.params.SHOW_COUPONS_BASKET === 'Y') {
                    this.editCoupons(basketContent, 'basket');
                    if (this.totalInfoBlockNode) {
                        var totalContent = this.totalInfoBlockNode.querySelector('.bx_ordercart_coupon');
                        this.editCoupons(totalContent, 'total');
                    }
                }
        ...
        createBasketItem: function (basketItemsNode, item, index, active) {
            ...
            if (this.options.showPreviewPicInBasket || this.options.showDetailPicInBasket)
                Columns.image = this.createBasketItemImg(item.data);

            Columns.title = this.createBasketItemContent(item.data);
            ...
            for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
                ...
                Columns[currentColumn.id] = this.createBasketItemColumn(currentColumn, item, active);
            ...
            cols.push(Columns.image);
            cols.push(
                BX.create('DIV', {
                    props: {className: 'bx-soa-div'},
                    children: [ Columns.title, Columns['QUANTITY'] ]
                })
            );

            delColumn = BX.create('DIV', {
                props: {className: 'bx-soa-item-td bx-soa-item-properties bx-text-right'},
                html: '<a class="bx-delete" href="javascript:void(0);" data-id='+item.data['ID']+'>Удалить</a>',
                events: {click: BX.proxy(this.deleteItem, this)}
            });
            cols.push(
                BX.create('DIV', {
                    props: {className: 'bx-soa-div'},
                    children: [ Columns['SUM'], delColumn ]
                })
            );
        ...
        createBasketItemImg: function (data) {
            ...
            logoNode = BX.create('DIV', {
                props: {className: 'bx-soa-item-td bx-soa-item-img-block'},
                children: [logoNode]
            });
                ...
                for (var i in props) {
                    if (props.hasOwnProperty(i)) {
                        ...
                        if (name == 'Product_Link') {
                            BX.addClass(logoNode, 'img-small');
                        }
                    }
                }
            }
        ...
        createBasketItemContent: function (data) {
            ...
            if (this.options.showPropsInBasket && props.length) {
                for (var i in props) {
                    if (props.hasOwnProperty(i)) {
                        ...
                        //except for the service property
                        if ((name != 'Product_Link') && (name != 'Product_Set')) {
                            propsNodes.push(
                                BX.create('DIV', {
                                    props: {className: 'bx-soa-item-td-title'},
            ...
            //this.options.showAddonsInBasket
            if (addons.length) {
                for (var i in addons) {
                    if (addons.hasOwnProperty(i)) {
                        var name = addons[i].NAME || '',
                            price = addons[i].PRICE_FORMATED || '';

                        if (price == 0) {
                            //var aname = 'Огурцы марин 50г, 1 шт'; 
                            //var aprice = 'Бесплатно';
                            addonsNodes.push(
                                BX.create('DIV', {
                                    props: {className: 'addons-row'},
                                    children: [
                                        name,
                                        BX.create('SPAN', {
                                            props: {className: 'price-free'},
                                            text: price,
                                        })
                                    ]
                                })
                        ...
                        //var aname = 'Болгарский перец 1 шт'; 
                        //var aprice = '50 руб';
                            addonsNodes.push(
                                BX.create('DIV', {
                                    props: {className: 'addons-row'},
            ...
            Nodes.push(BX.create('DIV', {props: {className: 'bx-soa-item-title'}, html: titleHtml}));
            if (addonsNodes.length) {
                Nodes.push(BX.create('DIV', {props: {className: 'addons-name'}, html: addonsHtml}));
                Nodes.push(BX.create('DIV', {children: addonsNodes}));
            }
            if (propsNodes.length)
                Nodes.push(BX.create('DIV', {props: {className: 'bx-scu-container'}, children: propsNodes}));

            return BX.create('DIV', {
                props: {className: 'bx-soa-item-td bx-soa-item-content'},
                children: Nodes
            });
        },
        ...
        createBasketItemColumn: function (column, allData, active) {
            ...
            if (column.id === 'PRICE_FORMATED') {
                ...
                //textNode.appendChild(BX.create('DIV', {html: data[column.id]}));
                this.editQuantity(textNode, allData.data);
        ...
        editQuantity: function(textNode, data) {

            textNode.appendChild(
                BX.create('DIV', {
                    props: {className: 'itQuant_container'},
                    children: [
                        BX.create('A', {
                            props: {
                                href: 'javascript:void(0)',
                            ...
                            events: {
                                click: BX.delegate(function (e) {
                                    this.setQuantity(data['ID'], 1, 'down', true);
                                }, this)
                            }
                        }),
                        BX.create('INPUT', {
                            ...
                            events: {
                                change: BX.delegate(function (e) {
                                    this.updateQuantity('QUANTITY_INPUT_'+data['ID'], data['ID'], 1, true);
                                }, this)
                            }
                        }),
                        BX.create('A', {
                            ...
                            events: {
                                click: BX.delegate(function (e) {
                                    this.setQuantity(data['ID'], 1, 'up', true);
                                }, this)
                            }
                        })
                    ]
        ...
        updateQuantity: function (controlId, basketId, ratio, bUseFloatQuantity)
        {
            ...
            if (bIsCorrectQuantityForRatio)
            {
                ...
                this.sendRequest('recalculate', basketId);
            }
            else
            {
        ...
        {
            var elem = BX("QUANTITY_INPUT_" + basketId);
            if (!elem) return;
            ...
            this.updateQuantity('QUANTITY_INPUT_' + basketId, basketId, ratio, bUseFloatQuantity);
        },
        ...
        editCoupons: function (basketItemsNode, mode = 'basket') {
            var couponsList = this.getCouponsList(true, mode),
                couponsLabel = this.getCouponsLabel(true),
                couponsBlock;

            if (mode == 'total') {
                for (var i = 0; i < couponsList.length; i++) {
                    basketItemsNode.appendChild(couponsList[i]);
                }
            } else {
        ...
        getCouponsList: function (active, mode = 'basket') {
            var couponsList = [], i;
        ...
        getCouponNode: function (coupon, active, mode = 'basket') {
                    ...
                    children: [
                        BX.create('INPUT', {
                            props: {
                                name: 'OLD_COUPON[]',
                                type: 'text',
                                className: 'good bx-soa-coupon-item-' + couponItem,
                                value: couponName,
                                disabled: '',
                                readonly: ''
                            }
                        ...
                        BX.create('span', {
                            ...
                            events: {
                                click: BX.delegate(function (e) {
                                    var coupon = e.target || e.srcElement;
        ...
        applyCoupon: function() {
            var elemCoupon = this.totalInfoBlockNode.querySelector('.bx-soa-coupon-input');
            if (elemCoupon && elemCoupon.value) {
                this.sendRequest('enterCoupon', elemCoupon.value);
                //elemCoupon.value = '';
            }
        },
        ...
        editActivePaySystemBlock: function (activeNodeMode) {
            ...
            if (this.initialized.paySystem) {
                ...
                if (this.params.SHOW_COUPONS_PAY_SYSTEM == 'Y')
                    this.editCoupons(paySystemContent, 'payment');
        ...
        createPaySystemItem: function (item) {
            ...
            itemNode = BX.create('DIV', {
                props: {className: 'bx-soa-pp-company col-lg-5 col-sm-5 col-xs-6'},
        ...
        editRestaurants: function (propsNode, propName) {
            ...
            BX.addClass(propsItemNode, "form-group bx-soa-customer-field restaurants");

            textHtml += BX.util.htmlspecialchars(propName);
            label = BX.create('LABEL', {
                attrs: {'for': 'soa-property-resto'},
                props: {className: 'bx-soa-custom-label'},
                html: textHtml
            });
            ...
            for (i in this.result.ADDRESS_RESTO) {
                if (this.result.ADDRESS_RESTO.hasOwnProperty(i)) {
                    current = this.result.ADDRESS_RESTO[i];
                    options.push(BX.create('OPTION', {
                        attrs: {
                            'data-city': current.CITY,
                        },
                        props: {
                            value: current.ID_ZONE,
                            selected: current.ID_ZONE == this.result.ORDER_RESTAURANTS
                        },
            ...
            var select = BX.create('SELECT', {
                ...
                children: options,
                events: {
                    change: BX.proxy(this.changeRestaurant, this)
                }
            });
        ...
        setRestaurant: function(zoneId, address, city) {
            ...
            if (this.yandexMap) {
                this.yandexZoneId = zoneId;
                this.getZone(zoneId);
                this.createMark(address, this.yandexMap, this.yandexMark, 'resto', "Ресторан");
            }
        },

        changeRestaurant: function (event) {
            var target, zoneId, address, city,
            addressClient, oldCity, newCity, 
            addrInput;

            target = event.target || event.srcElement;
            zoneId = target.value;
            address = target.options[target.selectedIndex].text;
            city = target.options[target.selectedIndex].getAttribute('data-city');

            oldCity = this.curRestaurants.city;
            this.setRestaurant(zoneId, address, city);
            newCity = this.curRestaurants.city;

            addrInput = BX('soa-property-7');
            addressClient = addrInput.value;

            if ( !this.changeCityInAddress(addressClient, oldCity, newCity) ) {
                //if address without city
                this.addedCityInClientAddress(addressClient);
            }
        },

        changeCityInAddress: function (address, oldCity, newCity) {
            var ind, addr, 
            addrInput;

            ind = address.indexOf(oldCity);
            if (ind >= 0) {
                addr = address.replace(oldCity, "");
                addr = newCity + addr;
        ...
        addedCityInClientAddress: function (addressClient) {
            var selResto, opt, attrCity, ind, addressClient, addrInput;

            selResto = BX('orderRestaurants');
            if (selResto && (selResto.selectedIndex >= 0)) {
                opt = selResto.options[selResto.selectedIndex];
                if (opt) {
                    attrCity = opt.getAttribute('data-city');
                    ind = addressClient.indexOf(attrCity);
                    if (ind < 0) {
                        addressClient = attrCity + ' ' + addressClient;

                        addrInput = BX('soa-property-7');
                        addrInput.value = addressClient; 
                    }
                }
            }
        },
        ...
        editActiveDeliveryBlock: function (activeNodeMode) {
            ...
            if (this.initialized.delivery) {
                ...
                if (this.params.SHOW_COUPONS_DELIVERY == 'Y')
                    this.editCoupons(deliveryContent, 'delivery');
        ...
        editDeliveryInfo: function (deliveryNode) {
            ...
            if(currentDelivery.PARENT_ID == 23){
                $("#bx-soa-orderSave a[data-save-button]").show();
            }
        ...
        editActivePropsBlock: function (activeNodeMode) {
            ...
            /*if (this.initialized.props !== undefined)
			{
				BX.remove(BX.lastChild(node));
            ...
            this.editRestaurants(propsNode, 'Рестораны');
            ...
            this.showAddressFromStorage();
            //this.radiusDostavkiMap();
        },

        showAddressFromStorage: function () {
            var addrInput, address;
            addrInput = BX('soa-property-7');

            if (addrInput && (addrInput.value == '')) {
                address = localStorage.getItem('bx-order-address');
                if (address != '') {
                    addrInput.value = address;
                    this.createMark(address,  this.yandexMap, this.yandexMark, 'client', "Клиент");
                }
            }
        },

        showBtnOrder: function ($TYPE) {
            var buttonSendTo = $('.pull-right.btn.btn-default');
            //var buttonMobile = $($('.btn.btn-default.btn-lg.btn-order-save')[1]);
            var buttonMobile = $('#bx-soa-total .btn-order-save, #bx-soa-total-mobile .btn-order-save');
            if ($TYPE == "HIDE") {
                buttonSendTo.css({
                    'display': 'none',
                })
        ...
        getCordsFromAdres: function ($string) {
            var $this = this;
            ...
            geocoder.then(
                function (res) {
                    ...
                    var myPlacemark = $this.createPlacemark(coordinates);
                    ...
                    myMap.geoObjects.splice(2, 3, myPlacemark);


                    var myMapPseudo = new ymaps.Map('yandexDeliveryPseydo', {
                        center: [55.74954, 37.621587],
                        zoom: 10
                    });

                    var myPolygon = new ymaps.Polygon([
                            myMap.DELIVERY_CORDS
                        ]);


                    myMapPseudo.geoObjects.add(myPolygon);
                    var resultSearch = myPolygon.geometry.contains(coordinates);

                    if (resultSearch ==true) {
                        $this.showBtnOrder("SHOW")
                    } else {
                        $this.showBtnOrder("HIDE")
                    }
...
//Начало моего кода Begin my code
        createYandexMap: function () {
            ...
            function init() {
                ...
                $mapObj = $("#yandexDeliveryZona");

                if (!$mapObj.hasClass("active_map")) 
                    $mapObj.addClass("active_map");

                $this.yandexMap = new ymaps.Map('yandexDeliveryZona', {
                    center: [55.751574, 37.573856],
                    zoom: 12,
                    controls: ['typeSelector','fullscreenControl','zoomControl',
                        'geolocationControl','trafficControl','rulerControl']
                });
        ...
        changeYandexMap: function () {
            this.getZone(this.yandexZoneId);
            
            if ((this.yandexMark['client']) &&
                ( $( 'input[name="DELIVERY_ID"]:checked' ).val() == this.result.DELIVERY_NO_ZONE )) {
                this.yandexMap.geoObjects.remove(this.yandexMark['client']);
                this.yandexMark['client'] = null;
            }
        },

        createMark: async function (address, yandexMap, mapMark, markName, markContent) {

            var coords,
            errMessage = 'Неверный адрес, попробуйте изменить адрес либо выберите другой ресторан';

            if (!yandexMap) {
                console.log('No yandex map');
                return;
            }
            if (address == '') {
                console.log('No address - ', markName);
                return;
            }

            if (mapMark[markName]) {
                yandexMap.geoObjects.remove(mapMark[markName]);
                mapMark[markName] = null;
            }
            ...
            try {
                ...
                console.log(err);
                this.showBtnOrder("HIDE");
                this.showValidationResult([BX('soa-property-7')], [errMessage]);
            }

            if (coords) {
                this.createYandexMark(yandexMap, coords, 
                    mapMark, markName, markContent);
            }
        },

        createYandexMark: function (yandexMap, coords, mapMark, markName, content) {

            mapMark[markName] = new ymaps.Placemark(coords, {
                iconContent: content},
                {
            ...
            yandexMap.geoObjects.add(mapMark[markName]);
        ...
        checkAddress: function (display_name, address, ) {
            var arr = [], find_name, regexp, isAddr = false;

            address.replace(/\\/g, '\\');
            arr = address.split(' ');
            arr = arr.reverse('?,\s');
            find_name = '(^|\\s)'+arr[0] + ',?,\\s' + arr[1] + '\\s(.+)?,\\s' + arr[2]+',?,';

            regexp = new RegExp(find_name);
            isAddr = regexp.test(display_name); 
        ...
        getCoordsByAddress: function(address) {
            ...
            return new Promise(function(resolve, reject) {
                ...
                var urlAjax;
                urlAjax = $this.templateFolder + '/../../' + "ajax/getCoordsAjax.php";
    
                $.ajax({
                    type: "POST",
                    url: urlAjax,
                    dataType: "html",
                    data: {"AJAX": "Y", 'Address': address},
                    success: function (data) {
                        var i, lat, lon, obj, 
                        item, coords, addrIndexes = [], j= 0,
                        display_name, isAddr;
                        ...
                        for (i = 0; i < obj.MESSAGE.length; i++) {
                            ...
                            isAddr = $this.checkAddress(display_name, address);
                        ...
                        if (lat && lon && isAddr) {
                            //if building and address is ok
                            coords = [lat,lon];
                            resolve(coords);
                        } else if ( addrIndexes.length > 0 ) {
                            //if address is ok
                            lat = obj.MESSAGE[addrIndexes[0]].lat;
                            lon = obj.MESSAGE[addrIndexes[0]].lon;
                            coords = [lat,lon];
                            resolve(coords);
                        } else if (obj.MESSAGE.length > 0) {
                            //if no address
                            lat = obj.MESSAGE[0].lat;
                            lon = obj.MESSAGE[0].lon;
                            coords = [lat,lon];
                            resolve(coords);
                        } else {
                            //if no coords
        ...
        IsPointInZones: function() {
            const err = 'Извините, мы не доставляем по этому адресу';
            var arrZones, mark, 
            coords, polygon;

            arrZones = this.yandexZones;
            if (arrZones) {
                mark = this.yandexMark['client'];

                if (mark) {
                    coords = mark.geometry.getCoordinates();

                    var result = ymaps.geoQuery(arrZones);
                    polygon = result.searchContaining(coords).get(0);

                    if (!polygon) {
                        //cancel order
                        console.log('---cancel order');
                        this.showError(this.mainErrorsNode, err);
                        this.showBtnOrder("HIDE");
                        return;
                    } else {
                        console.log('---ОК order');
                        this.resetErrMode();
                    }
                }
            }
        ...
        resetErrMode: function () {
            this.showBtnOrder("SHOW");
            var errorNode = this.mainErrorsNode.querySelector('.alert.alert-danger');
            errorNode.style.display = 'none';
        },


        findCityFromRestaurant: function(address, arrResto) {
            var item, current,
            ind, i, j=0;
            ...
            for (i in this.result.ADDRESS_RESTO) {
                ...
                ind = address.indexOf(current.CITY);
                if (ind >= 0) {
                    item = {
        ...
        initZone: async function () {
            var zoneId, address, city, addrClient, 
            selResto, index, arrResto = [],
            i, j, isDel;
            
            selResto = BX('orderRestaurants');

            addrClient = localStorage.getItem('bx-order-address');
            if (addrClient && (addrClient != '')) {
                this.findCityFromRestaurant(addrClient, arrResto);
                if (arrResto.length > 0) {

                    if (selResto.length > 0) {
                        for ( i = selResto.length-1; i >= 0; i-- ) {
                            isDel = true;
                            for (j = 0; j<arrResto.length; j++) {
                                if (selResto[i].text == arrResto[j].address) {
                                    isDel = false;
                                }
                            }
                            if (isDel) {
                    ...
                    if (selResto.length == 1) {
                        console.log('dropdown restaurants - one option');
                        //set zone by restoran address in dropdown 
                        zoneId = selResto.value;
                        address = selResto.options[selResto.selectedIndex].text;
                        city = selResto.options[selResto.selectedIndex].getAttribute('data-city');
                        this.setRestaurant(zoneId, address, city);
                        return;
                    }
                }
            ...
            index = await this.getClientLocation(this.yandexMap, 'browser', 'islands#blueCircleIcon');
            if (index >= 0) {
                selResto.selectedIndex = index;
        ...
        getZone: function (zoneId) {
            ...
            if (this.yandexZones) {
                this.yandexMap.geoObjects.remove(this.yandexZones);
                this.yandexZones = null;
            }

            if (( Number(zoneId) > 0 ) &&
                ( $( 'input[name="DELIVERY_ID"]:checked' ).val() != $this.result.DELIVERY_NO_ZONE )) {

                urlAjax = $this.templateFolder  + '/../../' + "ajax/getMapZone.php"; 
        ...
        createZone: function (data) {
            ...
            this.yandexZones = new ymaps.GeoObjectCollection();

            /*
            * добавим в коллекция yandexZone полигоны из массива data
            */
            for (var k = 0; k < data[0].delivery_areas.length; k++) {
                ...
                this.yandexZones.add(new ymaps.Polygon(areaCoord,
...
*/
            ...
            this.yandexMap.geoObjects
                .add(this.yandexZones);

            zoom = data[0].map.zoom;
            center = data[0].map.center;
            this.yandexMap.setCenter(center, zoom);
        },

        getClientLocation: function (yandexMap, prov, color) {
            ...
            return geolocation.get({
                ...
                //result.geoObjects.options.set('preset', color);
                //yandexMap.geoObjects.add(result.geoObjects);
                //console.log('OK '+ prov);
                return await $this.getMinDistance(result.geoObjects.position);
            }, async function (e) {
                // Если местоположение невозможно получить, то просто создаем карту.
                console.log('Error '+ prov);
                    
                // Красным цветом пометим положение, вычисленное через ip.
                return await $this.getClientLocation(yandexMap, 'yandex', 'islands#redCircleIcon');
            });
        },

        getMinDistance: async function (clientCoords) {
            var i = 0, j = 0, restoList, zoneId,
            resto, address, coords, item, minDist, minInd = -1,
            restaurants = [],
            errMessage = 'Неверный адрес, попробуйте изменить адрес либо выберите другой ресторан';

            restoList = BX('orderRestaurants');
            ...
            for(j = 0; j < restaurants.length; j++) {
                restaurants[j].dist = 
                    ymaps.coordSystem.geo.getDistance(clientCoords, 
                    restaurants[j].coords);

                if (!minDist || (minDist > restaurants[j].dist)) {
                    minDist = restaurants[j].dist;
                    minInd = j;
                }
            }

            if (minInd >= 0) {
                return restaurants[minInd].index;
            }
            return -1;
        },
        ...
        getAddress: function (myPlacemark, coords) {
            myPlacemark.properties.set('iconCaption', 'поиск...');
            ymaps.geocode(coords).then(function (res) {
                var firstGeoObject = res.geoObjects.get(0);
                var inputAdressDelivery = $('#soa-property-7');
                inputAdressDelivery.attr("value", firstGeoObject.getAddressLine())
                myPlacemark.properties
                    .set(
                        {
                            iconCaption: [firstGeoObject.getLocalities().length ? firstGeoObject.getLocalities() : firstGeoObject.getAdministrativeAreas(), firstGeoObject.getThoroughfare() || firstGeoObject.getPremise()].filter(Boolean).join(', '),
        ...
        yandexMapParams: function(parameters) {
            this.yandexMapInit = parameters.mapYandexDelivery;
            this.yandexMapId = parameters.mapYandexDeliveryId;
            this.yandexDeliveryСafe = parameters.mapYandexDeliveryСafe;
            this.yandexMap = null;
            this.yandexMark = [];
            this.yandexZones = null;
            this.yandexZoneId = null;
            this.curRestaurants = {};
        },
// Конец моего кода End my code
        yandexMapShow: function() {
            var $mapObj;
            var kyrier = false;

            $.each(this.result.DELIVERY, function (index, value) {
                if (value.CHECKED == "Y" && value.PARENT_ID == "24") {
                    kyrier = "Y";
                }
            });
            ...
            $mapObj = $("#yandexDeliveryZona");   
            if (!($.trim( $mapObj.html() ) === "")) {
                this.changeYandexMap();
            } else {
                this.createYandexMap();
            }

           //if (this.yandexMapInit * 1 > 0 && kyrier == "Y") {
                //this.createYandexMap();
            //} else {
            //    $(document).find("#yandexDeliveryZona").empty();
            //    $(document).find("#yandexDeliveryZona").removeClass("active_map");
            //    this.radiusDostavkiMap();
            //}
        },

        yandexMap2: function() {
            if (this.yandexMapInit * 1 > 0) {
            } else {
                $(document).find("#yandexDeliveryZona").empty()
                this.radiusDostavkiMap();
            }
        ...
        radiusDostavkiMap: function (address = false, city = false) {
           ...
           var $this = this;
            ...
            if (!address) {
                for (k in this.result.ORDER_PROP.properties) {
                    if (this.result.ORDER_PROP.properties[k].IS_ADDRESS == "Y" && this.result.ORDER_PROP.properties[k].VALUE[0].length) {
                        address = this.result.ORDER_PROP.properties[k].VALUE[0];
                    }
                }
            }
            if (!city) {
                for (k in this.result.ORDER_PROP.properties) {
                    if (this.result.ORDER_PROP.properties[k].CODE == "CITY" && this.result.ORDER_PROP.properties[k].VALUE[0].length) {
                        city = this.result.ORDER_PROP.properties[k].VALUE[0];
                    }
                }
            }
            ...
            var deliveryRadius = this.deliveryRadius,
                propsNode = this.paySystemBlockNode;

            if (!BX('map')) {
                var propsItemsContainer = BX.create('DIV', {
                    props: {className: 'bx-soa-section-content container-fluid'},
                    children: [
                        BX.create('DIV', {
                            props: {id: 'map'}
                        })
            ...
            var cord = this.coordinatsShopGps.split(','),
                pointMap = new google.maps.LatLng(cord[0], cord[1]),
                geo = new google.maps.Geocoder(),
                element = document.querySelector('[data-save-button]');
            ...
            google.maps.event.addListener(map, 'click', function (event) {
              ...
              var  messErr = [BX.message('DELIVERY_RADIUS_ERROR')];
                BX.Sale.OrderAjaxComponent.showError(BX.Sale.OrderAjaxComponent.propsBlockNode, messErr, true);
                BX.hide(element);
            });
            ...
            google.maps.event.addListener(circle, 'click', function (event) {
                placeMarker(event.latLng);
                BX.removeClass($this.propsBlockNode, 'bx-step-error');
                $this.showBlockErrors($this.propsBlockNode)
                BX.show(element);
            });
            ...
            if (address) {
                geo.geocode({'address': address}, function (results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                        var userCoords = results[0].geometry.location;
                        markerUser.setPosition(results[0].geometry.location)
                    } else {
                        messErr = [BX.message('ADDRESS_NOT_FOUND')];
                        BX.Sale.OrderAjaxComponent.showError(BX.Sale.OrderAjaxComponent.propsBlockNode, messErr, true);

                    }
                    var inRadius = false,
                        messErr = [BX.message('DELIVERY_RADIUS_ERROR')];
                    // var $rasstAtoB = google.maps.geometry.spherical.computeDistanceBetween(pointMap, userCoords);
                    ...
                    directionsService.route({
                        ...
                        if (status === 'OK') {
                            var $rasstAtoB = response.routes[0].legs[0].distance.value;

                            if (deliveryRadius * 1 > $rasstAtoB * 1) {
                                inRadius = true;
                                BX.show(element);
                                $this.showBlockErrors($this.propsBlockNode)
                            }
        ...
        getDeliveryType: function () {
            var i, item, parentId = 24;
            for (i in this.result.DELIVERY) {
                if (this.result.DELIVERY.hasOwnProperty(i)) {
                    item = this.result.DELIVERY[i];
                    if (item.CHECKED == "Y" && this.result.DELIVERY_PARENTS[item.ID] && this.result.DELIVERY_PARENTS[item.ID] == parentId)
                        return true;
                }
            }
            return false;
        ...
        editTotalBlock: function () {
            ...
            var elem = this.totalInfoBlockNode.querySelector('.button');
                    ...
                    BX.create('DIV', {
                        ...
                        children: [
                            BX.create('A', {
                                ...
                                html: 'Меню \
                                    <svg version="1.1" id="circle_back" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" \
                                    x="0px" y="0px" width="100%" height="100%" viewBox="0 0 612 612" xml:space="preserve"> \
...
$(document).on('change', '#soa-property-7', function () {
    var order = BX.Sale.OrderAjaxComponent;

    order.resetErrMode();
    var addressClient = $(this).val();
    localStorage.setItem('bx-order-address', addressClient);

    //if address without city
    order.addedCityInClientAddress(addressClient);

    order.createMark(addressClient,  order.yandexMap, order.yandexMark, 'client', "Клиент");
});

/*
$(document).on('change', '#soa-property-7', function () {
    var city = $('#soa-property-5').val();
    BX.Sale.OrderAjaxComponent.radiusDostavkiMap($(this).val(), city);
});

$(document).on('change', '#soa-property-5', function () {
    var address = $('#soa-property-7').val();
    BX.Sale.OrderAjaxComponent.radiusDostavkiMap(address, $(this).val());
});
*/